...
(deftest option-of-test
  (testing "returns an instance of Optional of value"
    (is (.equals (Optional/of 10) (op/option-of 10)))
    (is (.equals (Optional/of [10 11]) (op/option-of [10 11])))
    (is (.equals (Optional/of #{10 11}) (op/option-of #{10 11})))
    (is (.equals (Optional/of {:a 10 :b 11}) (op/option-of {:a 10 :b 11}))))
  (testing "returns an instance of empty Optional"
    (is (.equals (Optional/empty) (op/option-of)))))

(deftest has-value?-test
  (testing "returns true if Optional has value"
    (is (true? (op/has-value? (op/option-of 10)))))
  (testing "returns false if Optional is empty"
    (is (false? (op/has-value? (op/option-of))))))

(deftest =-test
  (testing "returns true if Optionals contain same value"
    (is (op/= (op/option-of 10) (op/option-of 10) (op/option-of 10) (op/option-of 10))))
  (testing "returns false if Optionals do not contain same value"
    (is (false? (op/= (op/option-of 10) (op/option-of 10) (op/option-of 10) (op/option-of 11))))
    (is (false? (op/= (op/option-of 10) (op/option-of))))))

(deftest get-test
  (testing "returns non-nil value held inside Optional"
    (is (= 10 (op/get (op/option-of 10))))
    (is (= [10 11] (op/get (op/option-of [10 11]))))
    (is (= #{10 11} (op/get (op/option-of #{10 11}))))
    (is (= {:a 10 :b 11} (op/get (op/option-of {:a 10 :b 11})))))
  (testing "returns nil value for empty Optional"
    (is (nil? (op/get (op/option-of)))))
  (testing "returns default value if supplied for empty Optional"
    (is (= 2 (op/get (op/option-of) 2)))))

(deftest filter-test
  (testing "returns Optional of value if predicate is satisfied"
    (is (op/= (op/option-of 10) (op/filter #(< 9 %) (op/option-of 10)))))
  (testing "returns empty Optional if predicate is satisfied"
    (is (op/= (op/option-of) (op/filter #(> 9 %) (op/option-of 10))))))

(deftest map-test
  (testing "returns Optional of value after application of fn"
    (is (op/= (op/option-of 10)
              (->> 9
                   op/option-of
                   (op/map inc)))
        "Transforms value held in option and returns Optional of resultant value")

    (is (op/= ((comp op/option-of
                     op/option-of) 10)
              (->> 9
                   op/option-of
    ...
    (is (op/= (->> (range 10) (map (comp op/option-of inc)) op/option-of)
              (->> (range 10)
                   (map op/option-of)
                   op/option-of
...
(deftest flat-map-test
  ...
  (let [expected-collection (->> (range 1 6) (map (partial * 2)) (map op/option-of))
...
(deftest optional-fn-return-test
  (testing "wraps a unsafe function and returns Optional of value or empty Optional"
    (let [log-fn (util/macro->fn log/info)
      ...
      (is (op/= (op/option-of) (wrapped-inc "9"))
